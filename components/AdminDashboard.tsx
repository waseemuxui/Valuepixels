...
import { LayoutDashboard, FileText, Settings, Search, Plus, BarChart3, Users, Globe, Save, Trash2, Edit, X, CheckSquare, ShoppingBag, DollarSign, ExternalLink, CheckCircle, XCircle, Sparkles, Loader2 } from 'lucide-react';
import { BlogPost, User, CustomPage, Product, Order, PaymentAccount, SiteConfig } from '../types';
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ user, posts, setPosts, pages, setPages, products, setProducts }) => {
  const [activeTab, setActiveTab] = useState<'overview' | 'posts' | 'pages' | 'shop' | 'payments' | 'settings' | 'ai-gen'>('overview');
  ...
  const [editType, setEditType] = useState<'post' | 'page' | 'product' | 'payment'>('post');
  ...
  return (
    <div className="min-h-screen bg-brand-dark pt-24 pb-12 px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row gap-8">
      ...
      <div className="flex-1">
        ...
        {activeTab === 'overview' && (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <StatCard title="Users" value={storage.getUsers().length} icon={Users} color="text-purple-400" />
            </div>
            {/* ... Analytics graph ... */}
          </div>
        )}
                ...
                <div className="bg-brand-surface border border-white/10 rounded-2xl p-8 max-w-2xl">
                    <div className="space-y-6">
                        <div>
                            ...
                            <input 
                                type="text" 
                                className="w-full bg-brand-dark border border-white/10 text-white rounded-xl px-4 py-3 focus:border-brand-primary outline-none"
                        ...
                        {aiGenerating && (
                            <div className="space-y-2">
                                <div className="flex justify-between text-xs text-brand-primary font-bold uppercase">
                                    <span>Generating...</span>
                                    <span>{aiProgress}%</span>
                                </div>
                                <div className="h-2 bg-brand-dark rounded-full overflow-hidden">
                                    <div className="h-full bg-brand-primary transition-all duration-300" style={{ width: `${aiProgress}%` }}></div>
                                </div>
                            </div>
                        )}
                        ...
                        <button 
                            ...
                            className="w-full bg-gradient-to-r from-brand-primary to-brand-secondary text-white font-bold py-4 rounded-xl shadow-lg hover:shadow-brand-primary/30 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                        >
                        ...
                        <p className="text-xs text-gray-500 text-center">
                            Note: Generated posts will be saved as "Drafts" in the Blog Posts section. Requires API Key.
                        </p>
                    </div>
                </div>
             </div>
                    ...
                    <form onSubmit={handleSaveConfig} className="space-y-8">
                        ...
                        <div className="p-6 bg-brand-primary/5 rounded-2xl border border-brand-primary/20">
                            <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                                <Sparkles className="w-5 h-5 text-brand-primary" /> AI Configuration
                            </h3>
                             <div className="space-y-2">
                                ...
                                <div className="relative">
                                    <input 
                                        type="password" 
                                        value={siteConfig.aiApiKey || ''}
                                        onChange={e => setSiteConfig({...siteConfig, aiApiKey: e.target.value})}
                                        className="w-full bg-brand-dark border border-white/10 text-white rounded-xl pl-4 pr-4 py-3"
                                        placeholder="AIzaSy..."
                                    />
                                </div>
                                <p className="text-xs text-gray-500">Required for AI Consultant and Bulk Content Generator.</p>
                            </div>
                        </div>
                        ...
                        <div>
                            <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                                <Globe className="w-5 h-5 text-brand-primary" /> General Information
                            </h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    ...
                                    <input 
                                        ...
                                        className="w-full bg-brand-dark border border-white/10 text-white rounded-xl px-4 py-3"
                                    />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-sm font-bold text-gray-300">Tagline / Description</label>
                                    <input 
                                        type="text" 
                        ...
                        <div>
                            <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                                <Users className="w-5 h-5 text-brand-secondary" /> Contact Details
                            </h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                        ...
                        <div>
                            <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                                <Search className="w-5 h-5 text-brand-accent" /> SEO Configuration
                            </h3>
                        ...
                        <button 
                            type="submit" 
                            className="bg-brand-primary text-brand-dark px-8 py-4 rounded-xl font-bold hover:bg-emerald-400 transition-colors shadow-lg shadow-brand-primary/20 flex items-center gap-2"
                        >
        ...
        {activeTab === 'payments' && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                {/* ... existing payments implementation ... */}
                 <div className="flex justify-between items-center">
                    <h2 className="text-3xl font-bold text-white">Payments & Verification</h2>
                </div>
                ...
                <div className="bg-brand-surface border border-white/10 rounded-2xl p-6">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <button 
                            ...
                            className="text-sm bg-brand-primary text-brand-dark px-3 py-2 rounded-lg font-bold hover:bg-emerald-400"
                        >
                            + Add Method
                        </button>
                    </div>
                            ...
                            <div key={account.id} className="bg-brand-dark border border-white/5 p-4 rounded-xl flex justify-between items-start">
                                <div>
                                    <h4 className="font-bold text-white flex items-center gap-2">
                                        <span className="uppercase text-xs bg-white/10 px-2 py-0.5 rounded text-gray-300">{account.type}</span>
                                        {account.name}
                                    </h4>
                                    <p className="text-gray-400 text-sm mt-1">{account.identifier}</p>
                                    <p className="text-gray-500 text-xs mt-1">{account.instructions}</p>
                                </div>
                                <div className="flex gap-2">
                                    <button 
                                        ...
                                        className="p-1 text-brand-primary hover:bg-white/10 rounded"
                                    >
                                        <Edit className="w-4 h-4" />
                                    </button>
                                    <button 
                                        onClick={() => handleDeleteAccount(account.id)}
                                        className="p-1 text-red-500 hover:bg-white/10 rounded"
                                    >
                ...
                <div className="bg-brand-surface border border-white/10 rounded-2xl overflow-hidden">
                    <div className="p-6 border-b border-white/10">
                        <h3 className="text-xl font-bold text-white">Orders Pending Verification</h3>
                    </div>
                                ...
                                <tr key={order.id} className="hover:bg-white/5">
                                    ...
                                    <td className="p-4">
                                        {order.proofOfPayment ? (
                                            <a href={order.proofOfPayment} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-brand-secondary hover:underline text-sm">
                                                View Proof <ExternalLink className="w-3 h-3" />
                                            </a>
                                        ) : <span className="text-gray-500 text-xs">No proof uploaded</span>}
                                        {order.transactionId && <div className="text-xs text-gray-400 font-mono mt-1">Ref: {order.transactionId}</div>}
                                    </td>
                                    <td className="p-4">
                                        <span className="bg-yellow-500/20 text-yellow-400 px-2 py-1 rounded-full text-xs font-bold">Pending</span>
                                    </td>
                                    <td className="p-4">
                                        <div className="flex gap-2">
                                            <button 
                                                onClick={() => handleVerifyOrder(order.id, true)}
                                                className="flex items-center gap-1 bg-green-500/20 text-green-400 px-3 py-1.5 rounded-lg hover:bg-green-500/30 font-bold text-xs"
                                            >
                                                <CheckCircle className="w-3 h-3" /> Approve
                                            </button>
                                            <button 
                                                onClick={() => handleVerifyOrder(order.id, false)}
                                                className="flex items-center gap-1 bg-red-500/20 text-red-400 px-3 py-1.5 rounded-lg hover:bg-red-500/30 font-bold text-xs"
                                            >
           ...
           <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
             <div className="flex justify-between items-center">
                ...
                <button 
                    ...
                    className="flex items-center gap-2 bg-brand-primary text-brand-dark px-4 py-2 rounded-xl font-bold hover:bg-emerald-400 transition-colors"
                >
                    ...
                    <tbody className="divide-y divide-white/5">
                        {posts.map(post => (
                            <tr key={post.id}>
                                ...
                                <td className="p-4"><span className="text-xs font-bold text-brand-primary bg-brand-primary/10 px-2 py-1 rounded">{post.status}</span></td>
                                <td className="p-4 flex gap-2">
                                    <button onClick={() => {setEditType('post'); setCurrentPost(post); setIsEditing(true);}} className="text-brand-primary p-1 hover:bg-white/10 rounded"><Edit className="w-4 h-4"/></button>
                                    <button onClick={() => handleDeletePost(post.id)} className="text-red-500 p-1 hover:bg-white/10 rounded"><Trash2 className="w-4 h-4"/></button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
        ...
        {activeTab === 'pages' && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="flex justify-between items-center">
                    <h2 className="text-3xl font-bold text-white">Custom Pages</h2>
                    <button onClick={() => { setEditType('page'); setCurrentPage({}); setIsEditing(true); }} className="flex items-center gap-2 bg-brand-primary text-brand-dark px-4 py-2 rounded-xl font-bold hover:bg-emerald-400 transition-colors">
                ...
                {/* Page Table */}
                 <div className="bg-brand-surface border border-white/10 rounded-2xl overflow-hidden">
                    <table className="w-full text-left">
                        <thead className="bg-white/5 text-gray-400 text-sm uppercase">
                            <tr><th className="p-4">Title</th><th className="p-4">Slug</th><th className="p-4">Actions</th></tr>
                        </thead>
                        <tbody className="divide-y divide-white/5">
                            {pages.map(page => (
                                <tr key={page.id}>
                                    ...
                                    <td className="p-4 flex gap-2">
                                        <button onClick={() => {setEditType('page'); setCurrentPage(page); setIsEditing(true);}} className="text-brand-primary p-1 hover:bg-white/10 rounded"><Edit className="w-4 h-4"/></button>
                                        <button onClick={() => handleDeletePage(page.id)} className="text-red-500 p-1 hover:bg-white/10 rounded"><Trash2 className="w-4 h-4"/></button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
        ...
        {activeTab === 'shop' && (
             <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="flex justify-between items-center">
                    ...
                    <button onClick={() => { setEditType('product'); setCurrentProduct({}); setIsEditing(true); }} className="flex items-center gap-2 bg-brand-primary text-brand-dark px-4 py-2 rounded-xl font-bold hover:bg-emerald-400 transition-colors">
                        ...
                        <tbody className="divide-y divide-white/5">
                            {products.map(p => (
                                <tr key={p.id}>
                                    ...
                                    <td className="p-4 flex gap-2">
                                        <button onClick={() => {setEditType('product'); setCurrentProduct(p); setIsEditing(true);}} className="text-brand-primary p-1 hover:bg-white/10 rounded"><Edit className="w-4 h-4"/></button>
                                        <button onClick={() => handleDeleteProduct(p.id)} className="text-red-500 p-1 hover:bg-white/10 rounded"><Trash2 className="w-4 h-4"/></button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
            ...
            <div className="relative w-full max-w-4xl bg-brand-surface border border-white/10 rounded-3xl shadow-2xl p-8 animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold text-white capitalize">
                        {currentAccount.id || currentPost.id || currentPage.id || currentProduct.id ? 'Edit ' : 'Add New '}
                        {editType}
                    </h2>
                ...
                <form onSubmit={editType === 'post' ? handleSavePost : editType === 'page' ? handleSavePage : editType === 'payment' ? handleSaveAccount : handleSaveProduct} className="space-y-6">
                        ...
                        <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-2">
                                    ...
                                    <select 
                                        value={currentAccount.type || 'payoneer'} 
                                        onChange={e => setCurrentAccount({...currentAccount, type: e.target.value as any})}
                                        className="w-full bg-brand-dark/50 border border-white/10 text-white rounded-xl px-4 py-3"
                                    >
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input 
                                        ...
                                        className="w-full bg-brand-dark/50 border border-white/10 text-white rounded-xl px-4 py-3"
                                        placeholder="e.g. Corporate Payoneer"
                                    />
                                </div>
                            </div>
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-gray-300">Identifier (Email/ID/IBAN)</label>
                                <input 
                                    type="text" 
                                    value={currentAccount.identifier || ''} 
                                    onChange={e => setCurrentAccount({...currentAccount, identifier: e.target.value})}
                                    className="w-full bg-brand-dark/50 border border-white/10 text-white rounded-xl px-4 py-3"
                                    placeholder="e.g. admin@sitefix.com"
                                />
                            </div>
                            <div className="space-y-2">
                                ...
                                <textarea 
                                    ...
                                    onChange={e => setCurrentAccount({...currentAccount, instructions: e.target.value})}
                                    className="w-full bg-brand-dark/50 border border-white/10 text-white rounded-xl px-4 py-3"
                                    placeholder="e.g. Please include Order ID in description."
                                />
                            </div>
                        </div>
                    )}
                    ...
                    {editType === 'post' && (
                         <>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    <label className="text-sm font-bold text-gray-300">Post Title</label>
                                    <input 
                                        type="text" 
                                        required
                                        value={currentPost.title || ''} 
                                        onChange={e => setCurrentPost({...currentPost, title: e.target.value})}
                                        className="w-full bg-brand-dark/50 border border-white/10 text-white rounded-xl px-4 py-3 focus:ring-2 focus:ring-brand-primary outline-none"
                                    />
                                </div>
                                 <div className="space-y-2">
                        ...
                        <>
                            ...
                            <div className="flex gap-6">
                                <label className="flex items-center gap-2 cursor-pointer text-gray-300 hover:text-white">
                                    <input 
                                        type="checkbox" 
                                        checked={currentPage.showInHeader || false}
                                        onChange={e => setCurrentPage({...currentPage, showInHeader: e.target.checked})}
                                        className="w-5 h-5 rounded border-gray-600 bg-gray-700 text-brand-primary focus:ring-brand-primary"
                                    />
                                    Show in Header Menu
                                </label>
                                <label className="flex items-center gap-2 cursor-pointer text-gray-300 hover:text-white">
                                    <input 
                                        type="checkbox" 
                                        checked={currentPage.showInFooter || false}
                                        onChange={e => setCurrentPage({...currentPage, showInFooter: e.target.checked})}
                                        className="w-5 h-5 rounded border-gray-600 bg-gray-700 text-brand-primary focus:ring-brand-primary"
                                    />
                                    Show in Footer Menu
                                </label>
                            ...
                            <div className="space-y-2">
                                ...
                                <textarea 
                                    ...
                                    className="w-full bg-brand-dark/50 border border-white/10 text-white rounded-xl px-4 py-3 focus:ring-2 focus:ring-brand-primary outline-none font-mono text-sm"
                    ...
                    <div className="flex justify-end gap-4 pt-4 border-t border-white/10">
                        ...
                        <button 
                            type="submit"
                            className="bg-brand-primary text-brand-dark px-8 py-3 rounded-xl font-bold hover:bg-emerald-400 transition-colors shadow-lg shadow-brand-primary/20"
                        >
                            Save
                        </button>
                    </div>
